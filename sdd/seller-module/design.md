...
export const SellerStatusTransitions: Record<
  SellerStatusValue,
  SellerStatusValue[]
> = {
...
- Seller `name` is UNIQUE (enforced in Prisma schema)
- Products from banned sellers are hidden from public catalog (filtered in queries)
- Products from suspended sellers remain visible
- Products from deleted sellers are hidden
