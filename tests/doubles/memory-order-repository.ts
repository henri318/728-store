import { OrderEntity, OrderRepository, OrderLineItemEntity, OrderStatus } from '@/modules/orders/domain/order-repository';
...
async saveOrderLineItems(orderId: string, lineItems: OrderLineItemEntity[]): Promise<void> {
...
const lineItems = this.orderLineItems.filter(item => item.orderId === orderId);
